...
const carValue = async (req, res) => {
  try {
    const {carimage,docimage,idimage, amount, currency, insurancetype, duration, company, calcAmount,  email } = req.body;
    if (!carimage||!docimage||!idimage||!amount || !currency || !insurancetype || !duration || !company || !calcAmount || !email) {
      return res.send({
        message: "all fields are required",
        success: false,
      });
