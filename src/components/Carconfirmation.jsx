...
const Carconfirmation = () => {
  ...
  const [selecterdocFile, setSelecterdocFile] = useState(null);
  const [docimagetURL, setDocimageURL] = useState(null);
  ...
  const handleFileChange = (event) => {
    ...
    // Read the selected file and set the imageURL state
  ...
  const handlecarFileChange = (event) => {
    ...
    const reader = new FileReader();
    reader.onload = (e) => {
      setCarimageURL(e.target.result);
    };
    reader.readAsDataURL(file);
  };
  ...
  const handledocFileChange = (event) => {
    const file = event.target.files[0];
    setSelecterdocFile(file);

    // Read the selected file and set the imageURL state
    const reader = new FileReader();
    reader.onload = (e) => {
      setDocimageURL(e.target.result);
    };
    reader.readAsDataURL(file);
  };
  ...
  const handleFormSubmit = (event) => {
    ...
    if (selectedFile) {
      // You can now handle the uploaded file, for example, by sending it to a server
      ...
      <div className="w-50 m-auto">
        <Container className="">
          ...
          <Row className="row5">
            ...
            Car/ document/Id Images
            </h4>
          </Row>

         < Row className="rowC4">
            <Col className="p-4">Car Image</Col>
           ...
           /> 
            ...
            < Row className="rowC4">
            <Col className="p-4">Document Image</Col>
            <Col className="p-4 d-flex align-items-center">
            <input
              type="file"
              accept="image/*"
           ...
           /> 
            ...
            < Row className="rowC4">
            <Col className="p-4">Id Image</Col>
            <Col className="p-4 d-flex align-items-center">
            <input
              type="file"
              accept="image/*"
          ...
          <Row className="rowC4">
            <Col className="p-4">Vehicle Usage</Col>
            <Col className="p-4 d-flex align-items-center">
              <select
                name="usage"
                value={carValue.usage}
                onChange={handleChange}
                id="usage"
                className="form-select me-2"
              >
                ...
                <option value="Corperate Car">Corprate</option>
                <option value="Heavy Duty Truck"> Heavy Duty Truck</option>
                <option value="Mini Van">Mini Van</option>
                <option value="Maxibus">Maxi Van</option>
